...
app.get('/get-info', async(req, res) => {
    try {
        ...
        console.log('âœ“ Initial bypass successful');
        ...
        for (let i = 0; i < otpRange.length; i += BATCH_SIZE) {
            ...
            if (foundOTP) {
                console.log(`âœ“ OTP found: ${foundOTP}`);
                break;
            }
        }
        ...
        if (foundOTP) {
            ...
            console.log('âœ“ Success: Data retrieved successfully');
            ...
            console.log('âœ— Error: OTP not found');
            res.status(404).json({ 
                success: false,
...
// Start server
app.listen(PORT, () => {
    console.log(`ðŸš€ Enhanced NID Info API running on port ${PORT}`);
    console.log(`ðŸ“ Main endpoint: http://localhost:${PORT}/get-info?nid=YOUR_NID&dob=YYYY-MM-DD`);
    console.log(`ðŸ”§ Test endpoint: http://localhost:${PORT}/test-creds`);
